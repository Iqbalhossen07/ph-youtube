...
const displayCategories = (categories) => {
    ...
    for (let cat of categories) {
        ...
        <button class="btn btn-sm onclick="loadCategoryVideos(${cat.category_id})" hover:bg-[#FF1F3D] hover:text-white">${cat.category}</button>
        <button class="btn btn-sm hover:bg-[#FF1F3D] hover:text-white" onclick="loadCategoryVideos(${cat.category_id})">${cat.category}
</button>
        `;
